...
export class V2Media {
  ...
  private enqueue(gameId: string, work: () => Promise<void>): Promise<void> {
    const next = (this.pending.get(gameId) ?? Promise.resolve()).then(work).catch(() => { console.warn('voice_service_unavailable'); });
    this.pending.set(gameId, next);
    void next.finally(() => { if (this.pending.get(gameId) === next) this.pending.delete(gameId); });
    return next;
  }
