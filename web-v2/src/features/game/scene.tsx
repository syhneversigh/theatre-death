...
export function GameScene({ view, catalog, online, remaining, refresh, onExit, onExpired }: {
  ...
  const taskSignature = availableTasks.map(taskKey).join('|');
  useEffect(() => {
    const keys = new Set(mountedView.current.tasks.map(taskKey));
    setDrafts(old => Object.fromEntries(Object.entries(old).filter(([key]) => keys.has(key))));
  }, [taskSignature]);
  ...
  const setDraft = (value: ActionDraft) => { if (task) setDrafts(old => ({ ...old, [taskKey(task)]: value })); };
  ...
  const sendActive = () => { if (task && !locked && !actionIssue(view, task, draft)) void commands.submit(commandIntent(view, task, draft, crypto.randomUUID())); };
  ...
  return <div className="game-scene">
    ...
    </div><GameSidebar view={view} catalog={catalog} onIdentity={() => setOverlay({ kind: 'identity' })} onRules={() => setOverlay({ kind: 'rules' })} onEvent={event => setOverlay({ kind: 'event', event })}/></div>
    ...
    {overlay?.kind === 'rules' && <RulesBook catalog={catalog} context={context} onClose={() => setOverlay(null)}/>}
